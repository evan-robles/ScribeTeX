...
def _doc_with(sections):
    ...
    for title, subs in sections:
        subtext = "".join(subsection_block(st, f"body {st}", d) for st, d in subs)
...
def test_existing_note_labels():
    ...
    assert existing_note_labels(doc) == ["2025-01-01", "2025-02-01"]
