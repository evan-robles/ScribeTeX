...
def build_brief() -> str:
    ...
    return (
        ...
        "- CONFIDENCE: if a word, symbol, number, or sub/superscript is genuinely "
        "illegible or you are guessing, wrap ONLY that uncertain span in "
        "\\uncertain{...} (it renders underlined/orange so the user can check it "
        "against the original). Do NOT silently guess — a marked guess is "
        "recoverable, a silent one is lost. Use it sparingly, only for real "
        "uncertainty, not for clearly-legible text.\n"
        "\n"
        ...
        "\n"
        "MANDATORY FIGURE-CAPTURE PASS (do this — dropping a drawing loses "
        "information the user cannot recover):\n"
        "  After transcribing the text, go through the page images ONE PAGE AT A "
        "TIME. For EACH page, decide whether it contains any non-text mark — a "
        "drawing, sketch, diagram, chart, arrow, labelled figure, or graph. If it "
        "does, you MUST call save_figure for each such region and \\includegraphics "
        "it into the LaTeX at the right place. Text-only pages need no figure. Do "
        "NOT finish while any page still has an uncaptured drawing.\n"
        "  Report, per page, whether a figure was present and how many you captured "
        "(the caller enforces this: a page with a figure present but zero captured "
        "is treated as an INCOMPLETE transcription, not a finished one).\n"
        "\n"
